...
struct GroupedMaskedSchedulerInputSwapAB {
    uint32_t shape_n;
    int* grouped_layout;
};
...
struct GroupedWithOffsetSchedulerInput {
    uint32_t shape_m;
    int* grouped_layout;  // problem_m_offsets;
};

struct GroupedWithOffsetSchedulerInputSwapAB {
    uint32_t shape_m;
    int* grouped_layout;  // problem_n_offsets;
};
...
struct GroupedMaskedSchedulerSwapAB
{
    ...
    using Input = GroupedMaskedSchedulerInputSwapAB;
    Input input;
    ...
    __device__ __forceinline__ uint32_t get_global_m_idx(const uint32_t shape_dim, const uint32_t block_size, uint32_t const& block_idx, uint32_t const& n_block_idx = 0) {
    ...
    __device__ __forceinline__ uint32_t get_global_scales_a_idx(const uint32_t shape_dim, const uint32_t block_size, uint32_t const& block_idx, uint32_t const& n_block_idx = 0) {
        return curr_group_idx * shape_dim + block_idx * block_size;
    }
    ...
    __device__ __forceinline__ bool get_next_block(uint32_t& m_block_idx, uint32_t& n_block_idx) {
        ...
        while (true) {
            ...
            auto cur_size = static_cast<uint32_t>(__ldg(grouped_layout + curr_group_idx));

            n_offset = curr_group_idx * shape_n;
            n_boundary = n_offset + cur_size;
            num_n_blocks = ceil_div(cur_size, BLOCK_N);
...
struct GroupedWithOffsetScheduler
{
    ...
    uint32_t curr_cumsum;
    uint32_t m_offset;
    uint32_t m_padded_4_offset;
    uint32_t m_boundary;
    int* problem_m_offsets;
    ...
    {
        this->problem_m_offsets = input.grouped_layout;
        curr_group_idx = 0;
        curr_cumsum = 0;
    }
...
struct GroupedWithOffsetSchedulerSwapAB
{
    ...
    uint32_t curr_cumsum;
    uint32_t n_offset;
    uint32_t n_padded_4_offset;
    uint32_t n_boundary;
    int* problem_n_offsets;
    ...
    {
        this->problem_n_offsets = input.grouped_layout;
        curr_group_idx = 0;
        curr_cumsum = 0;
    }
...
struct SchedulerSelectorSwapAB {
    static constexpr auto select_type() {
        static_assert(GT == GemmType::Normal || GT == GemmType::GroupedWithOffset || GT == GemmType::GroupedMasked,
            "Only Normal, GroupedWithOffset, GroupedMasked are supported for SwapAB");
        if constexpr (GT == GemmType::Normal)
