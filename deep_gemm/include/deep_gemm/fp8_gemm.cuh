...
#pragma unroll
            ...
            if constexpr (SchedulerType::gemm_type == GemmType::GroupedWithOffset || SchedulerType::gemm_type == GemmType::GroupedMasked) {
...
public:
    ...
    static void run(__nv_bfloat16* gmem_d, float* scales_b, int* grouped_layout,
        ...
        using SchedulerType = typename SchedulerSelector<kGemmType, SHAPE_N, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K, kNumGroups, kNumTMAMulticast>::type;
        ...
        typename SchedulerType::Input scheduler_input;
        scheduler_input.shape_m = shape_m;
        scheduler_input.grouped_layout = grouped_layout;

        // Launch
        auto status = cudaLaunchKernelEx(&config, kernel,
                                         gmem_d, scales_b, grouped_layout,
                                         shape_m,
                                         tma_a_desc, tma_b_desc, tma_scales_a_desc, tma_d_desc, scheduler_input);
    ...
    static void run_swap_ab(__nv_bfloat16* gmem_d, float* scales_b, int* grouped_layout,
        ...
        if constexpr (kGemmType == GemmType::Normal || kGemmType == GemmType::GroupedMasked) {
            scheduler_input.shape_n = shape_m;
        } else {
            scheduler_input.shape_m = shape_m;
        }
        scheduler_input.grouped_layout = grouped_layout;
        ...
        auto status = cudaLaunchKernelEx(&config, kernel,
                                         gmem_d, scales_b, grouped_layout,
                                         shape_m,
                                         tma_a_desc, tma_b_desc, tma_scales_b_desc, tma_d_desc, scheduler_input);
        DG_HOST_ASSERT(status == cudaSuccess);
    }
    ...
    template <typename T>
    static CUtensorMap make_tma_scales_a_offset_desc(T* global_address, uint32_t shape_m, uint32_t num_groups) {
        auto max_m_padded_total = compute_padded_offset(shape_m, num_groups);

        return make_2d_tma_desc(global_address, Layout::ColMajor,
                                max_m_padded_total, ceil_div(SHAPE_K, BLOCK_K), BLOCK_M, 1,
                                CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
    }

    template <typename T>
    static CUtensorMap make_2d_tma_a_desc_swap_ab(T* global_address, uint32_t shape_m) {
        return make_2d_tma_desc(global_address, Layout::RowMajor,
    ...
    template <typename T>
    static CUtensorMap make_2d_tma_b_desc_swap_ab(T* global_address, uint32_t shape_n) {
        return make_2d_tma_desc(global_address, Layout::ColMajor,
                                SHAPE_K, shape_n * (kGemmType == GemmType::GroupedMasked ? kNumGroups : 1),
                                BLOCK_K, BLOCK_N);
    }

    template <typename T>
    static CUtensorMap make_2d_tma_d_desc_swap_ab(T* global_address, uint32_t shape_m) {
        return make_2d_tma_desc(global_address, Layout::RowMajor,
                                shape_m * (kGemmType == GemmType::GroupedMasked ? kNumGroups : 1), SHAPE_N, min(BLOCK_N, shape_m), min(BLOCK_M, SHAPE_N),
                                CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
    }

    template <typename T>
    static CUtensorMap make_2d_tma_scales_b_desc_swap_ab(T* global_address, uint32_t shape_n) {
        // Make TMA aligned to 16 bytes
        constexpr uint32_t kAlignment = 16 / sizeof(T);
        shape_n = ceil_div(shape_n, kAlignment) * kAlignment;

        return make_2d_tma_desc(global_address, Layout::RowMajor,
                                ceil_div(SHAPE_K, BLOCK_K) * (kGemmType == GemmType::GroupedMasked ? kNumGroups : 1), shape_n, 1, BLOCK_N,
                                CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
    }

    template <typename T>
    static CUtensorMap make_tma_scales_b_offset_desc_swap_ab(T* global_address, uint32_t shape_m, uint32_t num_groups) {
        auto max_n_padded_total = compute_padded_offset(shape_m, num_groups);

        return make_2d_tma_desc(global_address, Layout::RowMajor,
                                ceil_div(SHAPE_K, BLOCK_K), max_n_padded_total, 1, BLOCK_N,
                                CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
    }
